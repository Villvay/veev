...
function parse_user_agent( $u_agent = null ) {
	...
	if ( $browser == 'Iceweasel' ) {
		...
		$browser  = 'Browser';
	} else if ( $find('Kindle Fire', $key) || $find('Silk', $key) ) {
		...
		$browser = 'Midori';
		$version = $result['version'][$key];
	} else if ( $browser == 'MSIE' || ($rv_result && $find('Trident', $key)) || $find('Edge', $ekey) ) {
		$browser = 'MSIE';
	...
	} else if ( $find('Vivaldi', $key) ) {
		$browser = 'Vivaldi';
		$version = $result['version'][$key];
	} else if ( $find('Valve Steam Tenfoot', $key) ) {
		$browser = 'Valve Steam Tenfoot';
		$version = $result['version'][$key];
	} else if ( $find('Chrome', $key) || $find('CriOS', $key) ) {
		$browser = 'Chrome';
		$version = $result['version'][$key];
		...
		if ( ($platform == 'Android' && !($key = 0)) ) {
			...
			$browser = 'BlackBerry Browser';
		} else if ( $find('Safari', $key) ) {
			$browser = 'Safari';
		} else if ( $find('TizenBrowser', $key) ) {
